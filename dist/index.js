...
async function main() {
    ...
    if (oldFiles.length === 0) {
        await sendTelegramMessage(`🧹 <b>Uploadcare cleanup</b>\nNo files older than ${OLDER_THAN_DAYS} days found. Nothing to delete.`);
        return;
    }
    const result = await deleteUploads(oldFiles);
    const totalSizeMb = (oldFiles.reduce((sum, f) => sum + (f.size || 0), 0) / (1024 * 1024)).toFixed(2);
    const lines = [
...
main().catch(async (error) => {
    ...
    try {
        await sendTelegramMessage(`❌ <b>Uploadcare cleanup failed</b>\n${error instanceof Error ? error.message : String(error)}`);
    }
    catch {
